    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
      <q-card flat bordered>
        ...
        >
            ...
            <q-td :props="props" class="text-center">
              <q-btn-group spread>
                <q-btn
                    ...
                    color="primary"
                    icon="edit"
                    @click="openModal(props.row.index)"
                >
                ...
                <q-btn
                    ...
                    size="sm"
                    color="primary"
                    :to="subsetUrl.replace(':id', props.row._id)"
                    icon="remove_red_eye"
                >
                ...
                <q-btn
                    v-show="canConfirm"
                    v-if="props.row.isConfirmed == false"
                    @click="
                    activeConfirm = true;
          ...
          <template v-slot:body-cell-ProDuct="props">
            <q-td :props="props" class="text-center">
              <q-btn-group spread>
                ...
                <q-tooltip v-close-popup>{{props.row.firstName + ' ' + props.row.lastName }}</q-tooltip>
          ...
          <template v-slot:body-cell-academ="props">
            <q-td :props="props" class="text-center">
              <q-btn-group spread>

                <q-btn
                    v-show="hasSubsetAcademy"
                    size="sm"
                    color="primary"
                    :to="subsetUrlAcademy.replace(':id', props.row._id)"
                >
                آموزشگاه                  
                </q-btn>
                <q-tooltip v-close-popup>{{props.row.managerName}}</q-tooltip>

              </q-btn-group>
            </q-td>
          </template>

            <template v-slot:body-cell-schol="props">
            <q-td :props="props" class="text-center">
              <q-btn-group spread>

                <q-btn
                    v-show="hasSubsetSchool"
                    size="sm"
                    color="primary"
                    :to="subsetUrlSchool.replace(':id', props.row._id)"
                >
                مدرسه                  
                </q-btn>
                <q-tooltip v-close-popup>{{props.row.managerName}}</q-tooltip>
            ...
            <q-td :props="props" class="text-center">
              <q-btn-group spread>
                <q-btn
                    ...
                    size="sm"
                    color="primary"
                    :to="subsetUrlFrequentlyQuestion.replace(':id', props.row._id)"
                    icon="remove_red_eye">
          ...
          <q-card-section>
            <div class="q-pa-md">
              <div class="row q-col-gutter-xs">
                <div
                    v-for="(eachForm, index) in form.filter(
                    (x) => x.type == 'file'
                  )"
                    :key="eachForm.field"
                  ...
                  >
                    <template v-if="eachForm.isExternal" v-slot:before>
                      <q-avatar>
                        <q-icon
                            name="open_in_new"
                            @click="opemExternal(eachForm.src)"
                ...
                >
                  <component
                      v-if="isEdit"
                      :maxlength="eachForm.maxlength"
                      :is="eachForm.type"
                      :label="eachForm.label"
                      :options="eachForm.options"
                      :value="eachForm.model"
                      :url="eachForm.url"
                      @input="updateValue(eachForm.field, $event)"
                      :disable="eachForm.disable"
                      emit-value
                      map-options
                      filled
                      autogrow
