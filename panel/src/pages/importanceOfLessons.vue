...
export default {
  ...
  mounted() {
    this.table= {
      ...
      readonly: false,
      hasSubset: true,
      subsetUrl: "/importanceOfLessons/:id",
      title: "لیست اهمیت دروس",
      filter: "",
      mode: "list",
      url: `model/importanceOfLessons`,
      columns: [
        {
          name: "index",
          label: "#",
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axios
        .get("model/importanceOfLessons",{headers: config})
        .then((response) => {
          response.data.data.items.forEach((row, index) => {
            row.index = index + 1;
          });
          this.table.data=response.data.data.items.filter((x)=>!x.parent);
