...
export default {
    ...
    this.table = {
      ...
      columns: [
       ...
       
        {
          ...
          align: "left",
          label: "وضعیت تسویه",
          field: (row) => row.status === "pending" ? "درحال پرداخت" : row.status === "accept" ? "پرداخت شده" : row.status === "reject" ? "رد شده" : "",
          sortable: true,
        },
        {
          name: "actions",
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axios
        .get(`financial/withdrawal/${this.$route.params.itemId}`,{headers: config})
        .then((response) => {
