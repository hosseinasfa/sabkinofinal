...
export default {
  ...
  mounted() {
    this.table = {
      ...
      readonly: false,
      hasSubset: true,
      subsetUrl:'/itemReport/postQuestion/:id',
      title: `لیست گزارشات سوالات پرسمان `,
      filter: "",
      mode: "list",
      url: `model/report`,
      columns: [
...

        {
          ...
          align: "left",
          label: "عکس",
          field: (row) => row.postId.avatar?? 'نامشخص',
          sortable: true,
        },

        {
          name: "firstName",
          align: "left",
          label: "سوال گزارش شده",
          field: (row) => row.postId.caption,
          sortable: true,
        },
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axios
        ...
        .then((response) => {
            ...
            var Fres = res.filter(e => e.type === "postQuestion"  && e.Handling === true);
    ...
    getList2() {
      ...
      this.$axios
        ...
        .then((response) => {
            var res = response.data.data.items;
            var Fres = res.filter(e => e.type === "postQuestion" && e.Handling === false);
