...
export default {
  ...
  mounted() {
    this.$store.commit("publicData/setTable", {
      ...
      readonly: false,
      hasSubset: true,
      subsetUrl: "news/postCategory/:id",
      title: "لیست کانال اخبار ",
      filter: "",
      mode: "list",
      url: `model/postCategory`,
      columns: [
        {
          name: "index",
          label: "#",
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axios
        .get(`model/postCategory`,{headers: config})
        .then((response) => {
          var FilRes = response.data.data.items.filter((x)=>x.type === "News");
          FilRes.forEach((row, index) => {
            row.index = index + 1;
          });

          this.table.data = FilRes;
