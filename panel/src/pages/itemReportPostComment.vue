...
export default {
  ...
  mounted() {
    this.table = {
      ...
      readonly: false,
      hasSubset: true,
      subsetUrl:'/itemReport/postComment/:id',
      title: `لیست گزارشات پاسخ های پرسمان `,
      filter: "",
      mode: "list",
      url: `model/report`,
      columns: [
        ...
        },
        {
          ...
          field: (row) => row.reportUserId.firstName,
          sortable: true,
        },
        {
          name: "lastName",
          ...
          field: (row) => row.reportUserId.lastName,
          sortable: true,
        },
        {
          name: "lastName",
          ...
          field: (row) => row.reportUserId.class === "user" ? "دانش آموز" : row.reportUserId.class === "mentor" ? "مشاور" : row.reportUserId.class === "teacher" ? "معلم" : row.reportUserId.class === "family" ? "خانواده" : row.reportUserId.class === "schoolBoss" ? "مدیر مدرسه" : "",
          sortable: true,
        },

        {
          name: "caption",
          align: "left",
          label: "پاسخ گزارش شده",
          field: (row) => row.postCommentId.caption,
          sortable: true,
        },

        {
          ...
          align: "left",
          label: "عکس پاسخ",
          field: (row) => row.postCommentId.AnswerImage,
          sortable: true,
        },
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axios
        ...
        .then((response) => {
            ...
            var Fres = res.filter(e => e.type === "postComment" && e.Handling === true);
    ...
    getList2() {
      ...
      this.$axios
        ...
        .then((response) => {
            var res = response.data.data.items;
            var Fres = res.filter(e => e.type === "postComment" && e.Handling === false);
