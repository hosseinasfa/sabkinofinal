...
export default {
  ...
  mounted() {
    this.table = {
      ...
      readonly: false,
      hasSubset: true,
      subsetUrl: "/categories/petSub/:id/options",
      title: "",
      filter: "",
      mode: "list",
      url: `panel/categories/petSub/${this.$route.params.petId}`,
      columns: [
        {
          name: "index",
          label: "#",
  ...
  methods: {
    ...
    getList() {
      this.$axios
        .get(`panel/categories/petSub/${this.$route.params.petId}`)
        .then((response) => {
          this.table.title =`زیر دسته های ${response.data.data.title}`; 
