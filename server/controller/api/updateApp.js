...
const Model = require("../../model/updateApp");
...
module.exports = {
    ...
    GET_APP_UPDATE: (req, res, next) => {
        Model.findOne({isActive:true }).sort('id').exec(function (err, doc) {
            if (doc) {
                Model.find({}).sort('id').exec(function (err2, docs) {
                    if (docs) {
                        var items = [];
                        docs.forEach(item => {
                            items.push({
